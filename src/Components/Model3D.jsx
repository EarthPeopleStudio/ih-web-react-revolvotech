...
function Model({ url }) {
  ...
  // Memoize allowed cursor animations for performance
  const allowedCursorAnimations = useMemo(
    () => [DEFAULT_ANIMATION, "Idle", "Shrug", "Grin"],
    [DEFAULT_ANIMATION]
  );
  ...
  // Find and store reference to the head, neck and torso bones (memoized)
  React.useEffect(() => {
    const bones = {
      head: null,
      neck: null,
    ...
    neckBoneRef.current = bones.neck;
    torsoBoneRef.current = bones.torso;
  }, [scene]);
    ...
    if (prevAnimationRef.current !== currentAnimation) {
      ...
      if (newAction) {
        if (previousAction && previousAction !== newAction) {
          if (
            previousAction.loop === THREE.LoopPingPong &&
            previousAction.isRunning()
          ) {
            const normalizedTime =
              (previousAction.time % previousAction.getClip().duration) /
  ...
  // Optimized material setup - only run once
  React.useEffect(() => {
    ...
    scene.traverse((child) => {
      if (child.isMesh && child.material) {
        ...
        if (materialCache.has(name)) {
          child.material = materialCache.get(name);
          return;
        }
        ...
        if (child.material.color) {
          switch (name) {
            ...
            case "Object_149": // Gold highlights
          ...
          // Force material update and cache it
          child.material.needsUpdate = true;
          materialCache.set(name, child.material);
        }
      }
    });
  }, [scene]);
  ...
  useFrame(() => {
    ...
    if (!allowedCursorAnimations.includes(currentAnimation)) return;

    const mouseX = mouseRef.current.x;
    const mouseY = mouseRef.current.y;

    // Pre-calculate rotation amounts for better performance
